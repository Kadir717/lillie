 ...
 * any field triggers the same submit path. The API rejects consumer
 * mailboxes (gmail.com, yahoo.com, ...), so a company email is required.
 *
...
export default function RecruiterSignupPage() {
  ...
  async function handleSubmit() {
    ...
    setSubmitting(true);
    try {
      const res = await fetch("/api/recruiter-auth/signup", {
        method: "POST",
      ...
      <div className="max-w-md mx-auto px-6 py-16 sm:py-24">
        ...
        <div className="mt-8 bg-cloud border border-line rounded-card p-8">
          ...
          <p className="mt-1 text-sm text-slate">
            For companies reviewing candidates. Recruiter accounts are
          ...
          <div className="mt-6 space-y-4">
            <div>
              <label htmlFor="companyName" className="block text-sm font-medium text-ink mb-1.5">
                Company name <span className="text-slate font-normal">(optional)</span>
              </label>
              <input
                id="companyName"
                type="text"
            ...
            <div>
              ...
              <p className="mt-1.5 text-xs text-slate">
                Consumer mailboxes (gmail.com, yahoo.com, ...) are not accepted.
              </p>
            </div>
        ...
        <p className="mt-6 text-sm text-slate text-center">
          ...
          <Link href="/recruiter/login" className="text-signal font-medium hover:text-signal/80 transition-colors">
            Sign in
          </Link>
        </p>
      </div>
